...
echo "===== 2mL VLM multi-task EvoRL queue start: $(date) ====="
echo "RUN_ID=$RUN_ID"
...

{
  ...
  echo "===== PI05 multi-task EvoRL full train: $(date) ====="
  "$PY" -m accelerate.commands.launch \
    --num_processes 8 \
    --num_machines 1 \
    --mixed_precision bf16 \
  ...
  echo "===== 2mL VLM multi-task EvoRL queue finished: $(date) ====="
