...
GPU_SET="${GPU_SET:-4,5}"
PORT="${PORT:-30441}"
RUN_ID="2mL_right_E299_prompt_v2_from_E249_evorl_action_expert_only_20260522_2gpu_5k"
...
echo "===== Action-expert-only E299 smoke: $(date), GPU_SET=$GPU_SET ====="
rm -rf "$SMOKE_OUT"
CUDA_VISIBLE_DEVICES="${GPU_SET%%,*}" "$PY" -m lerobot.scripts.lerobot_train \
...
echo "===== Action-expert-only E299 full train: $(date) ====="
if [ -e "$OUT" ]; then
...
echo "===== Action-expert-only E299 finished: $(date) ====="
echo "OUT=$OUT"
