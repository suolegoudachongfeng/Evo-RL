...
OUT=/mnt/project_eai/chp/checkpoints/policy_train_pi05_2mL_right_E249_prompt_v2_evorl_050000_dagger25_vision_lora_r16_20260521_ft_003000_4gpu
SMOKE_OUT=/mnt/project_eai/chp/checkpoints/smoke_pi05_2mL_right_E249_prompt_v2_evorl_050000_dagger25_vision_lora_r16_4gpu

cd "${EVO_RL_HOME}"

echo "===== DAgger vision-LoRA PI05 smoke: $(date) ====="
rm -rf "${SMOKE_OUT}"
CUDA_VISIBLE_DEVICES=0 "${PYTHON}" -m lerobot.scripts.lerobot_train \
  ...
  --job_name=smoke_pi05_2mL_right_E249_prompt_v2_evorl_050000_dagger25_vision_lora_r16_4gpu \
  --wandb.enable=false
rm -rf "${SMOKE_OUT}"

echo "===== DAgger vision-LoRA PI05 full 4-GPU fine-tune: $(date) ====="
if [ -e "${OUT}" ]; then
...
"${PYTHON}" -m accelerate.commands.launch \
  ...
  --acp.indicator_dropout_prob=0.3 \
  --output_dir="${OUT}" \
  --job_name=policy_train_pi05_2mL_right_E249_prompt_v2_evorl_050000_dagger25_vision_lora_r16_ft_003000_4gpu \
  --wandb.enable=false

echo "===== DAgger vision-LoRA PI05 fine-tune finished: $(date) ====="
