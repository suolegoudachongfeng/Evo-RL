#!/usr/bin/env python3
"""Combine three OPC segment datasets into one multi-task LeRobot dataset."""
...
from split_opc_dataset_by_vlm_segments import (
    SEGMENT_TASKS,
    _compute_stats,
...
SEGMENT_ORDER = ["open_drawer", "place_rack", "close_drawer"]
...
def _task_table(prompts: list[str]) -> pd.DataFrame:
    return pd.DataFrame({"task_index": np.arange(len(prompts), dtype=np.int64)}, index=pd.Index(prompts, name=None))
...
def combine_segments(*, split_root: Path, output_root: Path, video_mode: str, replace: bool) -> dict[str, Any]:
    segment_roots = {key: split_root / SEGMENT_TASKS[key]["dir"] for key in SEGMENT_ORDER}
    missing = [
        str(path)
        for path in segment_roots.values()
        if not (path / "data/chunk-000/file-000.parquet").is_file()
    ]
    if missing:
    ...
    pd.DataFrame([{"segment_key": key, **value} for key, value in segment_summary.items()]).to_csv(
        output_root / "combine_summary.csv", index=False
    )
    return report
