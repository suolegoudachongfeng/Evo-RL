#!/usr/bin/env python3
"""Split an OPC LeRobot dataset using VLM-produced three-stage timestamps."""
...
SEGMENT_TASKS = {
    "open_drawer": {
        "dir": "seg1_open_drawer",
        "prompt": "Open the instrument drawer. Stop once the drawer is open and stable.",
    },
    "place_rack": {
        "dir": "seg2_place_rack_in_drawer",
        "prompt": (
            "Place the rack into the correct position inside the open instrument drawer. "
            "Stop once the rack is released and stable."
        ),
    },
    "close_drawer": {
        "dir": "seg3_close_drawer",
        "prompt": "Close the instrument drawer. Stop once the drawer is closed and stable.",
    },
}
...
def _extract_segments(
    ...
    episode_time_bounds: dict[int, tuple[float, float]],
    *,
    pre_margin_s: float,
    post_margin_s: float,
    ...
    for ep_idx in sorted(episode_original):
        ...
        open_seg = segments.get("open_drawer", {})
        place_seg = segments.get("place_rack", {})
        close_seg = segments.get("close_drawer", {})

        boundary_open_done = open_seg.get("end_time")
        boundary_place_done = place_seg.get("end_time")
        conf_open = float(open_seg.get("confidence", 0.0) or 0.0)
        conf_place = float(place_seg.get("confidence", 0.0) or 0.0)
        conf_close = float(close_seg.get("confidence", 0.0) or 0.0)
        min_segment_conf = min(conf_open, conf_place, conf_close)
        if (
            boundary_open_done is None
            or boundary_place_done is None
            or min_segment_conf < min_confidence
            or float(boundary_place_done) <= float(boundary_open_done)
        ):
            failures.append(
                {
                    "episode_index": ep_idx,
                    "reason": "invalid_or_low_confidence_boundaries",
                    "drawer_open_stable_time": boundary_open_done,
                    "rack_released_stable_time": boundary_place_done,
                    "confidence": min_segment_conf,
                }
            )
            continue

        episode_start, episode_end = episode_time_bounds[ep_idx]
        boundary1 = float(boundary_open_done)
        boundary2 = float(boundary_place_done)
        by_key["open_drawer"].append(
            {
                "source_episode_index": ep_idx,
                "original_episode_index": episode_original[ep_idx],
                "start_time": episode_start,
                "end_time": max(episode_start, boundary1 + post_margin_s),
                "confidence": conf_open,
                "evidence": str(open_seg.get("evidence", "")),
            }
        )
        by_key["place_rack"].append(
            {
                "source_episode_index": ep_idx,
                "original_episode_index": episode_original[ep_idx],
                "start_time": max(episode_start, boundary1 - pre_margin_s),
                "end_time": max(boundary1, boundary2 + post_margin_s),
                "confidence": conf_place,
                "evidence": str(place_seg.get("evidence", "")),
            }
        )
        by_key["close_drawer"].append(
            {
                "source_episode_index": ep_idx,
                "original_episode_index": episode_original[ep_idx],
                "start_time": max(episode_start, boundary2 - pre_margin_s),
                "end_time": episode_end,
                "confidence": conf_close,
                "evidence": str(close_seg.get("evidence", "")),
            }
        )
...
def _build_one_dataset(
    *,
    ...
    pd.DataFrame({"task_index": [0]}, index=pd.Index([cfg["prompt"]], name=None)).to_parquet(
...
def main() -> None:
    ...
    episode_time_bounds = {
        int(ep_idx): (float(group["timestamp"].min()), float(group["timestamp"].max()))
        for ep_idx, group in src_data.groupby("episode_index", sort=False)
    }
